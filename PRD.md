...
- 缓存：Redis
- 任务队列：Celery
- 沙盒环境：Docker 容器或 Python 沙盒库（如 RestrictedPython）
...
- id, user_id, problem_description, input_data, output_data, agent_tree, status, created_at, updated_at
- agent_tree: JSON 格式存储完整的 Agent 执行树和时间线

**Agent 执行记录表 (agent_executions)**
- id, diagnosis_id, parent_agent_id, agent_type, status, start_time, end_time, input_tokens, output_tokens, cost

**工具调用记录表 (tool_calls)**
- id, agent_execution_id, tool_name, parameters, result, duration, created_at

**知识库表 (knowledge_base)**
- id, problem_type, problem_pattern, solution, created_by, created_at

**Skill 配置表 (skills)**
- id, name, version, file_path, config, created_at

**MCP 配置表 (mcp_servers)**
- id, name, endpoint, config, created_at

**LLM 配置表 (llm_configs)**
- id, name, api_endpoint, api_key, model_name, is_default, created_at
